...
def test_log_levels(level):
    ...
    with patch.object(sys, "argv", ["bogus", f"--log-level={level}", "test", "test", "test", "test", "test"]):
...
def test_bad_log_level():
    ...
    with patch.object(sys, "argv", ["bogus", "--log-level=emergency", "test", "test", "test", "test", "test"]):
        return_code = None
        try:
            tpt_reports.tpt_reports.main()
        except SystemExit as sys_exit:
